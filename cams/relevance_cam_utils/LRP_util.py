...
#############################################################################
class GradCAM_multi2(object):
    #GradCAM_multi2 for SIG CAM

    def __init__(self, model, target_layer, gradients_flag = False, g_flag = True, g_pp_flag = True, g_sig_flag = True):
        self.model = model
        self.gradients = dict()
        self.activations = dict()
        self.g_flag = g_flag
        self.g_pp_flag = g_pp_flag
        self.g_sig_flag = g_sig_flag
        self.gradients_flag = gradients_flag
        def forward_hook(module, input, output):
            self.activations['value'] = output
            return None
    ...
    def forward(self, input, class_idx=None, retain_graph=False):
        ...
        grad_cam, grad_cam_pp, sig_base_cam = 0,0,0
        ...
        if self.g_sig_flag:
            g_sig = np.mean(g * a, axis=(0, 1), keepdims=True).copy()
            sig_base_cam =a*g_sig
            sig_base_cam = np.sum(np.maximum(sig_base_cam,0),axis=-1)
            #sig_base_cam = cv2.resize(sig_base_cam,(224,224))

        if self.gradients_flag: return g_[0,0], a, grad_cam, grad_cam_pp,sig_base_cam
        else:                   return a, grad_cam, grad_cam_pp,sig_base_cam
