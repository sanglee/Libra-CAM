...
def preprocess_image(img, use_cuda=1, require_grad = False):
    ...
    print(preprocessed_img_tensor.shape)
    
    return Variable(preprocessed_img_tensor, requires_grad=require_grad)
    '''
    img = np.transpose(img, (2, 0, 1))
    preprocessed_img_tensor = transform(torch.tensor(img)).unsqueeze(axis=0)
    return preprocessed_img_tensor.cuda()
...
def get_heatmap(mask, img, blurred, blur_mask=0):
